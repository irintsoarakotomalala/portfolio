...
const Projects = () => {
  ...
  // Bloquer scroll arrière-plan quand galerie ouverte
  useEffect(() => {
    if (galleryProject) {
  ...
  useEffect(() => {
    const handleKey = (e: KeyboardEvent) => {
      if (galleryProject) {
        if (e.key === "ArrowLeft") changeGalleryIndex((galleryIndex - 1 + galleryProject.images.length) % galleryProject.images.length);
        if (e.key === "ArrowRight") changeGalleryIndex((galleryIndex + 1) % galleryProject.images.length);
  ...
  return (
    <section id="projects" className="section-padding overflow-visible" style={{ paddingBottom: `calc(1rem + ${DISC_SIZE / 2}px)` }}>
      <div className="max-w-7xl mx-auto">
        ...
        {/* Compteur + titre du projet EN DEHORS, au-dessus du carrousel */}
        <div className="flex items-end justify-between mb-4 gap-4">
          <AnimatePresence mode="wait">
            <motion.div key={activeIndex + "-counter"} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.3 }} className="flex items-baseline gap-3">
              <span className="text-5xl font-bold font-mono leading-none" style={{ color: COLOR }}>{String(activeIndex + 1).padStart(2, "0")}</span>
              <span className="text-lg font-mono text-white/25">/ {String(N).padStart(2, "0")}</span>
              <span className="text-sm font-mono text-white/40 ml-2 tracking-widest uppercase">{project.shortTitle}</span>
            </motion.div>
          </AnimatePresence>
          ...
          <div className="flex items-center gap-1.5 mb-1">
            {allProjects.map((_, i) => (
              <motion.button key={i} onClick={() => goTo(i)}
                ...
                style={{ height: 4, width: i === activeIndex ? 28 : 8, background: i === activeIndex ? COLOR : `${COLOR}30` }}
              />
            ))}
          </div>
        </div>
          ...
          >
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                />
                {/* Vignettage léger permanent */}
                <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(8,12,24,0.5) 0%, transparent 40%)" }} />
                <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(8,12,24,0.2) 0%, transparent 20%)" }} />
              </motion.div>
            </AnimatePresence>
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="absolute top-5 left-5 flex flex-wrap gap-2 z-20"
            ...
            {/* Overlay hover — couvre TOUT le conteneur, contenu centré */}
            <motion.div
              className="absolute inset-0 z-40 flex flex-col items-center justify-center px-10 text-center"
              style={{
                background: "rgba(8,12,24,0.78)",
                backdropFilter: "blur(2px)",
                WebkitBackdropFilter: "blur(2px)",
                pointerEvents: hovered ? "auto" : "none",
                paddingBottom: `${DISC_SIZE / 2 + 16}px`,
              }}
              initial={false}
              ...
              >
                ...
                <div className="flex items-center justify-center gap-3 flex-wrap mt-1">
                  ...
                  {project.images.length > 1 && (
                    <button
                      onClick={() => { setGalleryProject(project); setGalleryIndex(0); }}
                      className="group flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-bold border transition-all hover:scale-105"
                      style={{
                        background: `${COLOR}18`,
                        borderColor: `${COLOR}90`,
                        color: COLOR,
                        boxShadow: `0 0 14px ${COLOR}35, inset 0 0 8px ${COLOR}10`,
                      }}
                      onMouseEnter={(e) => (e.currentTarget.style.boxShadow = `0 0 24px ${COLOR}70, inset 0 0 12px ${COLOR}20`)}
                      onMouseLeave={(e) => (e.currentTarget.style.boxShadow = `0 0 14px ${COLOR}35, inset 0 0 8px ${COLOR}10`)}
                    >
                      <Images size={13} className="group-hover:scale-110 transition-transform" />
            ...
            >
              <h3 className="text-xl font-bold text-white/80 leading-tight drop-shadow-lg">{project.title}</h3>
            </motion.div>
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="relative flex items-center justify-center">
                  ...
                  <motion.div animate={{ scale: [1, 1.3, 1], opacity: [0.7, 1, 0.7] }} transition={{ duration: 2, repeat: Infinity }}
                    className="rounded-full z-10" style={{ width: 8, height: 8, background: COLOR, boxShadow: `0 0 12px ${COLOR}` }}
                  />
                  {[0, 60, 120, 180, 240, 300].map((deg, idx) => (
                    <motion.div key={deg} className="absolute"
                      style={{ transformOrigin: "center", transform: `rotate(${deg}deg) translateX(22px)` }}
              ...
              {/* Label tap */}
              <motion.div className="absolute left-1/2 -translate-x-1/2 pointer-events-none" style={{ bottom: "14%" }}
                animate={{ opacity: [0.4, 0.9, 0.4], y: [0, -2, 0] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }}
              >
          ...
          >
            ...
            <div className="absolute top-0 left-0 right-0 flex items-center justify-between px-6 py-4 z-10" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center gap-3">
                <span className="text-white font-semibold text-base">{galleryProject.title}</span>
                <span className="text-white/30 text-sm font-mono">{galleryIndex + 1} / {galleryProject.images.length}</span>
              </div>
              <div className="flex items-center gap-3">
                {/* Indicateur zoom */}
                <span className="text-xs font-mono px-2 py-1 rounded-lg border border-white/15 text-white/40" style={{ background: "rgba(255,255,255,0.05)" }}>
                  {Math.round(zoom * 100)}%
                </span>
                {zoom > 1 && (
            ...
            >
              ...
              {galleryProject.images.length > 1 && (
                <button
                  onClick={() => changeGalleryIndex((galleryIndex - 1 + galleryProject.images.length) % galleryProject.images.length)}
                  className="absolute left-4 z-20 w-11 h-11 rounded-full bg-black/50 hover:bg-black/80 text-white flex items-center justify-center transition-all hover:scale-110"
              ...
              {galleryProject.images.length > 1 && (
                <button
                  onClick={() => changeGalleryIndex((galleryIndex + 1) % galleryProject.images.length)}
                  className="absolute right-4 z-20 w-11 h-11 rounded-full bg-black/50 hover:bg-black/80 text-white flex items-center justify-center transition-all hover:scale-110"
                  style={{ border: `1px solid ${COLOR}40` }}
                >
              ...
              {zoom === 1 && (
                ...
                >
                  <span className="text-[11px] font-mono text-white/25 tracking-widest">↕ scroll pour zoomer</span>
                </motion.div>
              )}
            </div>

            {/* Barre basse — dots */}
            <div className="flex justify-center gap-2 py-4 z-10" onClick={(e) => e.stopPropagation()}>
              {galleryProject.images.map((_, idx) => (
                <button key={idx} onClick={() => changeGalleryIndex(idx)}
                  className="rounded-full transition-all duration-300 h-2"
                  style={{ width: idx === galleryIndex ? 28 : 8, background: idx === galleryIndex ? COLOR : "rgba(255,255,255,0.25)" }}
                />
              ))}
            </div>
          </motion.div>
